...
The diagonal elements predict the probability of an individual staying in the same size class; probabilities below the diagonal are when individuals contribute to the next larger size class through growth, and probabilities above the diagonal are for organisms shrinking or fragmenting. Hughes then applies this model to constrict a matrix for a population of reef coral, *Agaricia agaricites* and designates the following size classes: 0-10 cm^2^, 10-50 cm^2^, 50-200 cm^2^, and >200 cm^2^. Hughes constructed a matrix for each year of observation, because the second year experienced a severe storm that caused significant mortality amongst the coral population. Hence, the two matrices are described as "Calm" and "Storm", respectively, with the leading eigenvalues being the long-term growth rate, $\lambda$.
...
```
The equilibrium growth constant, $\lambda$~Calm~, is `r lambda.C`.
...
```
The equilibrium growth constant, $\lambda$~Stormy~, is `r lambda.S`.
...
Whilst it took approximately 35 years for the size structure to reach equilibrium under the calm matrix, the population stabilized much quicker under the storm matrix (in about 12 years) (Fig. 1). This is partly due to the fact that no individuals could reach class size IV. I am able to successfully confirm that once size class I declines due to mortality and growth, consecutive classes reach their peak cohort size before declining towards extinction.test = 
